...
[coefs,dinds,pcoefs] = bump2(xxgrid,yygrid,20,50,8,1e-8);
...
% Solve with GMRES
start = tic;
sol = gmres(@(mu) fast_apply_fft(mu,kerns,coefs,iinds,jinds,N2, 1),rhs_vec,[],1e-10,2000);
% sol = gmres(@(mu) fast_apply_fft(mu,kerns,coefs,iinds,jinds,N2, alpha(dinds) / a0),rhs_vec,[],1e-10,2000);
