...
L = 1.1e3; % length of grid
N1 = 1e3; % number of grid points

xs = linspace(-L/2,L/2,N1); % do not change
...
[coefs,dinds,pcoefs,H] = bumps(xxgrid,yygrid,-0.5e3,0.5e3,1/sqrt(64),zki,1e-12);
...
uincs = fggreen([0;0],[xxgrid(:) yygrid(:)].',rts,ejs);
phizinc = uincs(:,:,1)/2;
phiinc = uincs(:,:,end);
lapphizinc = uincs(:,:,5)/2;
uincs = uincs(dinds,:,:);
...
figure(2); clf 
pcolor(xxgrid,yygrid,H); shading interp;
colorbar
title('H')
drawnow
...
% Solve with GMRES
start = tic;
[sol,flag,relres,iter,resvec] = gmres(@(mu) fast_apply_fft(mu,kerns,coefs,iinds,jinds,N2),rhs_vec,[],1e-7,2000);
...
evalkerns = kerns(:,:,[1,8,5]);
evalspmats = {spmats{1},spmats{8},spmats{5}};
...
lapphiztot = lapphizsca + lapphizinc;

phiztot = reshape(phiztot,size(xxgrid));
phitot = reshape(phitot,size(xxgrid));
lapphiztot = reshape(lapphiztot,size(xxgrid));
...
nexttile
pc = pcolor(xxgrid,yygrid,abs(lapphiztot)); shading interp;
title('|\Delta\phi_z|')
colorbar
       
% Calculate error with finite difference
utots = cat(3,phiztot,phitot);
[abs_err,rel_err] = get_fin_diff_err(xxgrid,yygrid,utots,h,pcoefs,L/8,L/8,zk,dinds,'fg');
